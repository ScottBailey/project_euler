...
Brute force with num-bigint because 40ms is fast enough. I'm sure this
could be closer to 40 micros, but it doesn't need to be.
    ...
    for a in 2..=100_u64 {
        for b in 2..=100 {
            ...
            if let Err(ins) = v.binary_search(&num) {
                v.insert(ins,num); // only insert if the number wasn't found
            }
        }
    }
