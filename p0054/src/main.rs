...
fn solve() -> u64 {
    ...
    for line in reader.lines() {
        if let Ok(s) = line {
            ...
            let mut p1 = sb::poker::Hand::new();
            let mut p2 = sb::poker::Hand::new();
            for i in 0..10 {
                let offset : usize = 3*i;
                if i < 5 {
                    p1.add_card(s.as_bytes()[offset], s.as_bytes()[offset+1]);
                }
                else {
                    p2.add_card(s.as_bytes()[offset], s.as_bytes()[offset+1]);
                }
            }
